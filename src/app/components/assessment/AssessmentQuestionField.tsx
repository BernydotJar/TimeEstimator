...
export function AssessmentQuestionField({ question, onAnswer, onNotes, onAddEvidence, onRemoveEvidence }: Props) {
  ...
  return (
    <fieldset className="rounded-xl border border-slate-800 bg-slate-950/45 p-4">
      <legend className="px-1 text-sm font-semibold text-white">{question.prompt}{question.requiredForCompletion ? " *" : ""}</legend>
      ...
      <div className="mt-3 flex flex-wrap gap-2" aria-label={`Answer state for ${question.prompt}`}>
        {(["answered", "unknown", "not_applicable", "unanswered"] as AssessmentAnswerState[]).map((item) => (
          <Button key={item} type="button" size="sm" variant={state === item ? "default" : "outline"} onClick={() => onAnswer(item, item === "answered" ? answer?.value : undefined)}>{item.replaceAll("_", " ")}</Button>
        ))}
      </div>

      {state === "answered" && <div className="mt-3">
        {question.answerType === "long_text" ? (
          ...
          <select aria-label={question.prompt} className={inputClass} value={answer?.value === undefined ? "" : String(answer.value)} onChange={(event) => setValue(event.target.value === "" ? undefined : event.target.value === "true")}><option value="">Select…</option><option value="true">Yes</option><option value="false">No</option></select>
          ...
          <input aria-label={question.prompt} className={inputClass} type={question.answerType === "number" ? "number" : question.answerType === "date" ? "date" : "text"} value={String(answer?.value ?? "")} onChange={(event) => { const raw = event.target.value; setValue(question.answerType === "number" ? (raw === "" ? undefined : Number(raw)) : question.answerType === "entity_list" ? raw.split(",").map((item) => item.trim()).filter(Boolean) : raw); }} />
        )}
      </div>}

      <label className="mt-4 block text-xs font-medium text-slate-300">Notes<textarea className={inputClass} rows={2} value={answer?.notes ?? ""} onChange={(event) => onNotes(event.target.value)} /></label>
      <div className="mt-4 border-t border-slate-800 pt-3">
        <div className="flex flex-col gap-2 sm:flex-row">
          <select aria-label="Evidence kind" className={inputClass} value={evidenceKind} onChange={(event) => setEvidenceKind(event.target.value as EvidenceReference["kind"])}><option value="note">Note</option><option value="url">URL</option><option value="document_name">Document name</option><option value="workshop_statement">Workshop statement</option><option value="system_observation">System observation</option></select>
